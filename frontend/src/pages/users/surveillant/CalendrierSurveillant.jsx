...
import { getMySupervisions } from "../../../services/sideSurveillant";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <style>{`
        ...
        .animate-scaleIn { animation: scaleIn .14s ease-out; }
      `}</style>
    </div>
  );
}
...
export default function CalendrierSurveillant() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const res = await getMySupervisions();
        const list = Array.isArray(res) ? res : res?.data ?? [];
        setSupervisions(list);
      } catch (err) {
        console.error("fetch supervisions:", err);
        setSupervisions([]);
      } finally {
        setLoading(false);
      }
  ...
  const eventsByDay = useMemo(() => {
    ...
    for (const s of supervisions) {
      const exam = s.Exam ?? s.exam ?? null;
      let start = null;
      let end = null;

      if (s.start_time) {
        start = new Date(s.start_time);
        if (s.end_time) end = new Date(s.end_time);
        ...
        // skip supervision entries without a schedule
        continue;
      }
      ...
      if (!map[key]) map[key] = [];
      map[key].push({
        id: s.id ?? (exam ? `exam-${exam.id}` : undefined),
        start,
        end,
        exam,
        raw: s,
  ...
  return (
    <div className="space-y-6">
      ...
      {loading && <div className="text-sm text-slate-500">Chargement des supervisions...</div>}
    </div>
  );
}