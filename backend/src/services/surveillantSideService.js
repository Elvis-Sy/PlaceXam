...
import { CalendrierService } from "./calendrierService.js";
...
export class SurveillantService {
  ...
  static async getMyCalendriers(surveillantId) {
    if (!surveillantId) throw new Error("surveillantId requis");

    // récupérer les supervisions du surveillant (doit fournir examId / Exam)
    const supervisions = await SupervisionService.getBySurveillant(surveillantId);
    console.log("Supervision--------", supervisions)
    const examIds = Array.from(new Set((supervisions || []).map((s) => s.examId ?? s.Exam?.id).filter(Boolean)));

    if (examIds.length === 0) return [];

    // utiliser la nouvelle méthode qui applique la même logique persisté + virtuel
    return await CalendrierService.getCalendriersForExamIds(examIds);
  }
